    ...
    <div class="max-w-3xl mx-auto px-4 py-8" x-data="formDraft()" x-init="loadDraft()">
        ...
        <form action="{{ route('petani.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow space-y-4">
            ...
            <div>
                ...
                <input type="text" name="fruit_type" value="{{ old('fruit_type') }}" x-model="draft.fruit_type" @input="saveDraft" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 min-h-[44px] px-3 @error('fruit_type') border-rose-500 @enderror" required>
                @error('fruit_type') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Grade</label>
                <select name="grade" x-model="draft.grade" @change="saveDraft" class="mt-1 block w-full rounded-md border-gray-300 min-h-[44px] px-3 @error('grade') border-rose-500 @enderror" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" inputmode="decimal" step="0.01" min="0.1" name="estimated_weight_kg" value="{{ old('estimated_weight_kg') }}" x-model="draft.estimated_weight_kg" @input="saveDraft" class="mt-1 block w-full rounded-md border-gray-300 min-h-[44px] px-3 @error('estimated_weight_kg') border-rose-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" inputmode="numeric" min="100" name="price_per_kg" value="{{ old('price_per_kg') }}" x-model="draft.price_per_kg" @input="saveDraft" class="mt-1 block w-full rounded-md border-gray-300 min-h-[44px] px-3 @error('price_per_kg') border-rose-500 @enderror" required>
            ...
            <div>
                ...
                <input type="date" name="harvest_date" value="{{ old('harvest_date') }}" x-model="draft.harvest_date" @input="saveDraft" class="mt-1 block w-full rounded-md border-gray-300 min-h-[44px] px-3 @error('harvest_date') border-rose-500 @enderror" required>
