            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white border border-gray-150 rounded-3xl shadow-premium overflow-hidden">
                    <div class="px-6 py-5 border-b border-gray-100">
                        <h2 class="font-heading font-extrabold text-gray-800 text-base flex items-center gap-2">
                            ...
                            Aktivitas Estimasi Panen Terbaru
                        </h2>
                    </div>
                    <div class="p-0">
                        @if($recentEstimates->isEmpty())
                            <div class="px-6 py-12 text-center text-gray-400">
                                <p class="text-xs font-bold">Belum ada estimasi panen masuk.</p>
                            </div>
                        @else
                            <ul class="divide-y divide-gray-100">
                                @foreach($recentEstimates as $est)
                                <li class="px-6 py-4 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-3 min-w-0">
                                        <div class="h-9 w-9 rounded-xl bg-{{ $est->status_color }}-100 text-{{ $est->status_color }}-700 flex items-center justify-center text-sm font-extrabold shrink-0">🌾</div>
                                        <div class="min-w-0">
                                            <p class="text-sm font-extrabold text-gray-800 truncate">{{ $est->fruit_type }} — {{ $est->estimated_weight_kg }} Kg</p>
                                            <p class="text-[10px] text-gray-400 font-bold">{{ $est->user->name }} · {{ $est->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                    <span class="px-2.5 py-1 text-[9px] font-extrabold rounded-md uppercase bg-{{ $est->status_color }}-100 text-{{ $est->status_color }}-800 border border-{{ $est->status_color }}-200/50 shrink-0">
                                        {{ $est->status_label }}
                                    </span>
                                </li>
                                @endforeach
                ...
                <div class="bg-white border border-gray-150 rounded-3xl shadow-premium p-5 sm:p-6">
                    <div class="flex items-center justify-between border-b border-gray-100 pb-4 mb-4">
                        ...
                        <span class="bg-indigo-50 border border-indigo-200/50 text-indigo-700 text-xs font-extrabold px-3 py-1.5 rounded-xl">{{ $petaniLocations->count() }} Lahan</span>
                    </div>
            ...
            <div class="bg-white border border-gray-150 rounded-3xl shadow-premium overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-gray-50/50 to-white flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[10px] text-gray-400 mt-0.5">Mitra aktif</p>
                    </div>
                    ...
                    <ul class="divide-y divide-gray-100 max-h-[500px] overflow-y-auto pr-1">
                        ...
                        <li class="p-4 hover:bg-gray-50/50 transition flex items-center justify-between gap-4 font-semibold text-xs">
                            <div class="flex items-center min-w-0">
                                <div class="flex-shrink-0 h-9 w-9 rounded-xl bg-gradient-to-tr from-brand-600 to-emerald-500 text-white flex items-center justify-center font-extrabold text-sm shadow shadow-brand-500/10">{{ strtoupper(substr($petani->name, 0, 1)) }}</div>
                            ...
                            <div class="flex items-center gap-1.5 shrink-0">
                                <a href="{{ route('admin.petani.edit', $petani) }}" class="py-1 inline-flex items-center px-3 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl font-bold transition">Edit</a>
                                <form action="{{ route('admin.petani.destroy', $petani) }}" method="POST" onsubmit="return confirm('Hapus akun petani ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="py-1 inline-flex items-center px-3 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-100/50 rounded-xl font-bold transition">Hapus</button>
                                </form>
                            </div>
                        </li>
                        @empty
                        <li class="px-6 py-12 text-center text-gray-400 font-bold text-xs">☕ Belum ada petani terdaftar.</li>
                        @endforelse
                    </ul>
                </div>
            </div>
